...
BEGIN_DISPATCH_MAP(MemoryHelper, CWnd)
	...
	DISP_FUNCTION_ID(MemoryHelper, "GetHwndsByWindowName", dispidGetHwndsByWindowName, GetHwndsByWindowName, VT_BSTR, VTS_BSTR VTS_UI4)
	DISP_FUNCTION_ID(MemoryHelper, "FindSubWindow", dispidFindSubWindow, FindSubWindow, VT_UI4, VTS_UI4 VTS_UI4 VTS_BSTR VTS_BSTR)
END_DISPATCH_MAP()
...
BSTR MemoryHelper::GetHwndsByWindowName(LPCTSTR windowName, ULONG cmpType)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	while(tmpHwnd){
		CString titstr;
		tmpHwnd->GetWindowTextW(titstr);
		if (titstr.GetLength()>0 && ((titstr.CompareNoCase(windowName)==0 && cmpType==0) || (titstr.Find(windowName)>=0 && cmpType!=0)))
		{
			strResult.AppendFormat(L"%d|",tmpHwnd->m_hWnd);
		}
		tmpHwnd=tmpHwnd->GetNextWindow();
	}
...
ULONG MemoryHelper::FindSubWindow(ULONG hwnd1, ULONG hwnd2, LPCTSTR className1, LPCTSTR className2)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	return (ULONG)::FindWindowEx((HWND)hwnd1,(HWND)hwnd2,className1,className2);
}
